...
import { awsPaymentProcessorOperation } from '@/lib/amplify-client-wrapper';
...
const steps = [
  ...
  },
  {
    id: 'payment',
    title: 'Payment Setup',
    description: 'Set up your payment account for payouts',
    icon: CreditCard,
  },
  {
    id: 'verification',
...
export default function ProviderOnboarding() {
  ...
  const startPaymentOnboarding = async () => {
    try {
      setLoading(true);
      setError('');
      ...
      if (data.success) {
        // Move to next step - no external redirect needed
        setCurrentStep(2);
        checkPaymentStatus();
      } else {
  ...
  const openPaymentDashboard = async () => {
    try {
      ...
      // Redirect to provider dashboard payment section
      router.push('/provider/dashboard?tab=payments');
    } catch (err) {
      console.error('Error opening payment dashboard:', err);
        ...
        <Card>
          ...
          <CardContent className="space-y-6">
            ...
            {paymentStatus?.hasAccount && (
              <div className="space-y-4">
                <div className="flex items-center justify-between p-4 bg-muted rounded-lg">
                  ...
                  <Badge variant={paymentStatus.chargesEnabled ? 'success' : 'secondary'}>
                    {paymentStatus.chargesEnabled ? 'Active' : 'Pending'}
                  </Badge>
                </div>
                ...
                <div className="flex items-center justify-between p-4 bg-muted rounded-lg">
                  ...
                  <Badge variant={paymentStatus.detailsSubmitted ? 'success' : 'secondary'}>
                    {paymentStatus.detailsSubmitted ? 'Submitted' : 'Required'}
                  </Badge>
                </div>
                ...
                <div className="flex items-center justify-between p-4 bg-muted rounded-lg">
                  ...
                  <Badge variant={paymentStatus.payoutsEnabled ? 'success' : 'secondary'}>
                    {paymentStatus.payoutsEnabled ? 'Enabled' : 'Pending'}
                  </Badge>
                </div>
              </div>
            )}
            ...
            <div className="space-y-3">
              {!paymentStatus?.hasAccount && (
                <div className="space-y-4">
                  <Alert>
                    ...
                    <AlertDescription>
                      We use AWS Payment Cryptography to securely handle all payments. Your financial information 
                      is encrypted and protected with bank-level security.
                    </AlertDescription>
                  </Alert>
              ...
              {paymentStatus?.hasAccount && paymentStatus?.needsOnboarding && (
                <div className="space-y-4">
                  <Alert>
                    <AlertCircle className="h-4 w-4" />
                    <AlertDescription>
